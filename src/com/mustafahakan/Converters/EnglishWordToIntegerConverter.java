package com.mustafahakan.Converters;

import com.mustafahakan.Converters.Dictionaries.EnglishDictionary;

import java.util.Map;

import static java.lang.Integer.parseInt;

public class EnglishWordToIntegerConverter extends BaseWordToIntegerConverter {

    private Map<String, String> teens;

    // Constructor
    public EnglishWordToIntegerConverter() {
        super();
    }

    // Methods
    @Override
    protected void initMaps() {
        dictionary = new EnglishDictionary();
        super.initMaps();
        teens = reverseMap(((EnglishDictionary) dictionary).getTeens());
    }

    @Override
    protected int parseWord(String[] word_arr, int start, int end, int current, int total) {

        int N = end - start;

        if (N <= 0) {
            return current + total;
        }

        String first = word_arr[start];
        int value = 0;

        if (decimals.containsKey(first)) {
            value = (int) Math.pow(10, parseInt(decimals.get(first)));

            if (current == 0) {
                current = 1;
            }

            if (current < 1000) {
                current *= value;
            }

            if (current >= 1000) {
                total += current;
                current = 0;
            }
        }

        else {
            if (teens.containsKey(first)) {
                value = 10 + parseInt(teens.get(first));

            }

            if (ties.containsKey(first)) {
                value = 10 * parseInt(ties.get(first));
            }


            else if (digits.containsKey(first)) {
                value = parseInt(digits.get(first));
            }

            current += value;
        }

        return parseWord(word_arr, start + 1, end, current, total);
    }

}
